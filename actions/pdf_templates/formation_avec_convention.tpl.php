...
class PDF extends FPDF
{
	...
	function Footer()
	{
		...
		$pdf->SetFont('Arial','',10);
		$pdf->Write(5,' soit établie par l\'OCIM-Université de Bourgogne, merci de préciser');
		$pdf->SetFont('Arial','B',10);
		$pdf->Write(5,' toutes');
		$pdf->SetFont('Arial','',10);
		$pdf->Write(5,' les informations suivantes :');
		$pdf->Ln(6);
		$pdf->Write(8,'Nom, prénom et fonction du signataire :');
		$pdf->Ln(6);
		$pdf->Write(8,'Statut de l\'organisme :');
		$pdf->Ln(6);
		$pdf->Write(8,'Organisme et adresse complète :');
		$pdf->Ln(10);
		$pdf->SetFont('Arial','B',10);
		$pdf->Write(8,'Formule(s) ');
		$pdf->SetFont('Arial','',10);
		$pdf->Write(8,'(choisie(s) à la préinscription) :');
		$pdf->Ln(6);
		// récupération des formules choisies
